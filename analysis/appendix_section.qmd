...
```{r taxon-detail, eval=!is.na(taxon_detail), results = "asis"}
cat("Including ", knitr::combine_words(taxon_detail), ".", sep = "")
```
...
```{r fig-palaeodist, fig.height=16, fig.cap=paste0("Predicted palaeodistributions of *", taxon, "*")}
ggplot() +
  facet_wrap(
    vars(
  ...
  geom_sf(
    aes(shape = pred_present),
    data = archaeo |>
      mutate(
        period = climate_period, 
        pred_present = replace_na(pred_present, FALSE)
      ) |>
...
if (nrow(archaeo) > 0) {
  archaeo |>
    drop_na(climate_period) |>
    summarise(
      n_assemb = n(),
      mean_p = mean(prop),
      age_start = max(age_start),
      age_end = min(age_end),
      pred_present = all(pred_present),
    ...
    select(climate_period, site_name, age_start, age_end, n_assemb, mean_p, source,
           pred_present) |>
    gt(caption = paste0("Archaeological occurrences of *", taxon, "*")) |>

...
flora |>
  mutate(
    occ = map(occ, mutate, present = TRUE),
    bg_data = map(1:n(), \(i) { 
      background_sample(
        region = w_asia_land, 
        N = n_background,
...
flora |>
  mutate(
    fit = map(occ, \(occ) {
      fit(rf_model, factor(present) ~ ., data = select(occ, rf_predictors()))
    }, .progress = TRUE)
  ) ->
  flora
```
...
```{r predict}
# TODO: tidy
rf_fit <- filter(flora, taxon == "Triticum turgidum dicoccum") |>
  pull(fit)
rf_fit <- rf_fit[[1]]
...
rf_pred <- st_set_dimensions(rf_pred, "period", values = climate_periods)

...
Bioclimatic data – the standard, and what allows us to do hindcasting.

Topographic data derived from a DEM:

* Aspect and slope – proxies for solar radiation, but only if high resolution [@AustinVanNiel2011; @LeempoelEtAl2015]
* TWI – proxy for soil moisture, important for arid environments [@KopeckyCizkova2010; @CamposEtAl2016; DiVirgilioEtAl2018]

Soil data – important for plants [@DubuisEtAl2013; @ModEtAl2016; @VelazcoEtAl2017] but not all variables from SoilGrids are reliable: so we just use soil texture (clay, silt, sand) and pH at the surface (0-5 cm depth) [@MillerEtAl2024].
...
dem <- read_stars(derived_data("dem", "srtm_15plus_w_asia.tif"))
names(dem) <- "elevation"

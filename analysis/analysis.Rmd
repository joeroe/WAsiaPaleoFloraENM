...
```{r training-climate}
occ |>
  mutate(bio = extract_points(climate$cur, occ$geometry)) |>
  unpack(bio) ->
  occ
```
...
```{r model}
# TODO: Investigate where these NAs are coming from
...
```{r predict}
# TODO: deal with missing data in pred vars
# TODO: turn back to something spatial? stars?
predict(rf_fit, climate$cur)
```

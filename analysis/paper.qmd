...
This was not based on the region's prehistoric archaeological record, which at that point was barely known.
...
if (!file_exists(derived_data("flora.Rdata"))) {
  flora <- read_tsv(
    ...
    mutate(
      ...
      taxon = case_match(
        taxon,
        c("Bolboschoenus glaucus", "Bolboschoenus maritimus") ~ "Bolboschoenus",
...
The taxonomic identifications of archaeobotanical material given in our source databases were controlled to ensure consistency between sources and to group taxa that cannot always be reliably distinguished [for details see @ArranzOtaeguiRoe2023].
...
The aim in both cases was to resolve taxonomic ambiguity by using a more inclusive taxonomic unit.
For example, the model discussed here as *Bolboschoenus* includes both archaeological occurrences of *B. maritimus* and *B. glaucus*, since these were not distinguished by archaeobotanists until recently [@WollstonecroftEtAl2011], as well as modern occurrences under the older nomenclature *Scirpus maritimus* (see @tbl-results-summary).
...
@fig-occ-map shows clearly that the Southern Levant, and Israel specifically, is significantly more densely sampled than other regions.

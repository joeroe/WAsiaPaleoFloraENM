...
```{r dependencies, cache=FALSE, message=FALSE}
library("cowplot")
library("dplyr")
library("dotenv")
library("english")
...
library("WAsiaPaleoFloraENM") # this package, install with devtools::install()
```
...
if (!file_exists(derived_data("flora.Rdata"))) {
  flora <- read_tsv(
    ...
    drop_na(taxon) |>
    filter(taxon != "Medicago astroites") |> # Not reliably identifiable
    ...
    filter(
      n_archaeo >= min_n_archaeo | 
        taxon %in% c("Triticum urartu", "Aegilops speltoides", "Aegilops tauschii")
    )
...
```{r fig-region}
#| fig-cap: Map of the study region (West Asia, grey box) with locations of Late Epipalaeolithic and Pre-Pottery Neolithic archaeobotanical assemblages.
ggplot() + 
...
if (length(dir_ls(derived_data("gbif"))) == 0) {
  ...
  gbif_download <- occ_download(
    ...
    # pred_lt("coordinateUncertaintyInMeters", 1000), # too strict for W. Asia!
    format = "SIMPLE_CSV"
  ) |>
...
@misc{GBIFDataDownload,
  ...
  date = {2025-08-08},
  year = {2025},
  url = {https://doi.org/)", flora_occ_meta$doi, r"(}
})"), here("analysis/gbif.bib"))
...
The resulting cleaned dataset used to train our niche models comprises `r format(flora_occ_meta$totalRecords, scientific = FALSE)` occurrences from `r flora_occ_meta$numberDatasets` constituent datasets.

```{r fig-occ-map, message=FALSE, fig.cap=paste0("Georeferenced occurrence records from West Eurasia used to train models (N=", nrow(unnest(flora, occ)), "). Inset, right: prediction region (West Asia).")}
fig_occ_map <- flora |>
 unnest(occ) |>
...
fig_occ_map_inset <- fig_occ_map +
  coord_sf(
    crs = w_asia_albers, default_crs = latlong,
    xlim = w_asia[c(1,3)], ylim = w_asia[c(2,4)], expand = FALSE
  )

fig_occ_map <- fig_occ_map +
 layer_spatial(w_asia, fill = NA)

fig_occ_map + fig_occ_map_inset
```
...
```{r data-climate, warning=FALSE, message=FALSE}
...
climate <- read_stars(derived_data("climate.nc"), proxy = TRUE, quiet = TRUE)
...
```{r data-flora-predictors, warning = FALSE}
...
flora <- flora |> 
  mutate(
    occ = map(occ, st_as_sf, coords = c("longitude", "latitude"), crs = latlong, 
              remove = FALSE),
...
```{r niche-fit, cache.lazy=FALSE}
# Don't use lazy cache here: https://github.com/yihui/knitr/issues/572
...
```{r niche-predict, cache.lazy=FALSE}
...
if (!file_exists(derived_data("flora_niche_predictions.Rdata"))) {
  ...
  # Write predictions in TIF format
  palaeodist_dir <- dir_create(here("analysis", "palaeodist"))
  walk2(flora$taxon, flora$palaeodist, function(taxon, palaeodist) {
    filename <- paste0(str_replace(taxon, " ", "_"), ".tif")
    write_stars(
      select(palaeodist, "prediction.TRUE."), 
      path(palaeodist_dir, filename)
    )
  })
}
...
```{r data-model-assessment, cache.lazy=FALSE}
flora <- mutate(
  flora,
...
flora |>
  unnest(niche_test_metrics) |>
  filter(.metric == "sens") |>
  mutate(label = if_else(.estimate < 0.50, taxon, NA_character_)) |>
  ggplot(aes(n_occ, .estimate)) + 
  geom_smooth(method = "glm", formula = y ~ log(x), fullrange = TRUE, 
              se = FALSE) +
  geom_point() + 
  geom_text_repel(aes(label = label), fontface = "italic", size = 10/.pt) +
  scale_x_log10(name = "Occurrences", labels = label_number()) +
...
Model sensitivity is loosely correlated with the number of occurrences available for training (@fig-model-sensitivity-vs-n_occ), with the worst-performing models all having less than 1000 recorded occurrences: `r knitr::combine_words(paste0("*", filter(unnest(flora, niche_test_metrics), .metric == "sens" & .estimate < 0.50)$taxon, "*"))`.
Test metrics and ROC curves for the individual models are included in the appendix.

```{r niche-predict-archaeo, cache.lazy=FALSE}
  ...
  archaeo_pred <- flora |>
    ...
    mutate(
      ...
      archaeo = map2(archaeo, climate_period, function(x, period) {
        bind_extract(x, st_as_stars(hack(climate, "period", as.character(period))))
      }),
      archaeo = map(archaeo, bind_extract, st_as_stars(terrain)),
      archaeo = map(archaeo, bind_extract, st_as_stars(soil))
    ) |>
    unnest(archaeo) |>
...
The ability of the hindcast models to predict the occurrence of specific species at archaeological sites is worse, with just `r percent(p_archaeo_pred_correct)` of presences in archaeobotanical assemblages successfully predicted at a threshold of 0.5.
...
```{r niche-pred-range, cache.lazy=FALSE}
...
#| fig-cap: Distribution of predicted species niche size by period (logarithmic scale). Dashed lines indicate the median niche size.
flora |>
  unnest(area_present) |> 
  mutate(
    period_label = map(period, \(x) filter(climate_periods, code == x)$label),
  ...
  scale_x_log10(name = "Potential niche km²", labels = label_number_auto()) + 
...
`r n_reduced_range` of `r nrow(flora)` species are predicted reduced niches in the past; `r n_reduced_range_10` of more than 10% or more.
...
The average potential niche size of modelled species was `r percent(1 - (filter(flora_pred_area_present_summary, period == "eh")$med_area_present / filter(flora_pred_area_present_summary, period == "ba")$med_area_present))` smaller in the Early Holocene compared to the Bølling–Allerød, and `r percent(1 - (filter(flora_pred_area_present_summary, period == "yds")$med_area_present / filter(flora_pred_area_present_summary, period == "ba")$med_area_present))` smaller during the Younger Dryas.
...
```{r fig-palaeodist-barley-levant, fig.height=10}
#| fig-cap: Predicted palaeodistribution of wild barleys
...
bread <- c("Aegilops tauschii", "Triticum turgidum")
palaeodist_bread <- do.call(c, c(
  ...
  mutate(present2 = prediction.TRUE. > 0.2) |> # Need lower threshold for wheat
  st_apply(c("x", "y", "period"), sum, na.rm = FALSE) |>
  mutate(present2 = factor(present2))

ggplot() +
  facet_wrap(vars(fct_relabel(period, toupper))) +
  annotation_spatial(ne_land, fill = "white") +
  geom_stars(data = select(palaeodist_bread, present2), na.action = na.omit) +
  annotation_spatial(ne_land, fill = NA) +
  annotation_spatial(w_asia, fill = NA) +
...
In general there is middling to low agreement between our hindcast predictions of species niches and observed archaeological occurrences of the same species [@tbl-results-summary].

```{r fig-archaeo-sensitivity, message=FALSE, warning=FALSE}
#| fig-cap: Summary of hindcasting performance of models in relation to model performance and available archaeological test data
#| fig-height: 10
results_summary |>
  pivot_longer(c(n_archaeo, sens, n_occ), names_to = "metric") |>
  mutate(
    metric = case_match(
      metric,
      "n_archaeo" ~ "N occurrences (archaeological)",
      "n_occ" ~ "N occurrences (current)",
      "sens" ~ "Model sensitivity (current)"
    )
  ) |>
  ggplot(aes(value, p_predicted)) +
  ...
  labs(x = NULL, y = "Archaeological occurrences predicted") +
  theme_minimal_grid() +

...
abstract: |
  ...
  <!-- TODO: rewrite following reviews. Highlight BA to EH decline vs. 'amelioration' concept -->
...
flora <-  flora |>
  # Manually impute some problematic cases
  # mutate(
  #   occ = case_when(
  #     taxon_key %in% c(12225945, 7405743) ~ map(occ, function(x) {
  #       mutate(x, across())
  #     })
  #   )
  # ) |>
...
```{r fig-palaeodist-barely-levant, fig.height=10}
...
bread <- c("Aegilops tauschii", "Triticum aestivum", "Triticum turgidum")
...
ggplot() +
  ...
  annotation_spatial(ne_land, fill = "white") +
  geom_stars(data = select(palaeodist_bread, present), na.action = na.omit) +
  annotation_spatial(ne_land, fill = NA) +
  annotation_spatial(w_asia, fill = NA) +
...
The majority of our models perform poorly as a binary classification with a threshold of `P(present)>0.5`.
However, this is an arbitrary threshold; lowering it can improve sensitivity at the cost of specificity.
<!-- TODO: so why didn't we threshold? Losing information - stick with the probabilities. -->
In principle archaeological occurrences—as true presences—could be used to 'calibrate' the threshold, but there aren't enough archaeological attestations of most of the modelled species.

<!-- TODO: What about not binary? Mean P(T) and P(F) for example? -->

```{r fig-archaeo-sensitivity}
...
At this point we cannot explain precisely why this discrepency exists, though assessments of model performance offer some clues.
...
<!-- TODO: core issue - fundamental and realised niches, different but related signals -->

For a variety of reasons, our models probably underestimate the *fundamental* niche or potential distribution of the target taxon.
These include:

* The uneven coverage of the GBIF occurrence dataset (see @sec-occ-data)
* Use of time-averaged climate slices
...
* Time-averaged, sometimes quite severely
* (Understudied) possibility for human transplanting and transport; we know this is the dawn of agriculture!
* Generally small sample sizes, so individual misidentifications have a large impact 

So what can we do about it???

<!-- TODO: rewrite following reviews -->
...
Discrepancies between the modelled distribution, whether on the broad scale (e.g. the restricted geographic range of most species compared to their attestation in the archaeological record), or relating to specific species (i.e. false positives and false negatives), suggest several avenues for future investigation.

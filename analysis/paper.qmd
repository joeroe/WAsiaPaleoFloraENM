---
title: Biogeography of crop progenitors and wild plant resources in the Late Pleistocene and Early Holocene of West Asia, 21–8.3 ka
date: today
author: 
- name: Joe Roe
  id: jr
...
abstract: |
  ...
  the Early Holocene compared to present conditions, but did [not] change as
  drastically between the Last Glacial Maximum and the Early Holocene. 
  However, the modelled ranges do not predict the observed occurrence of these 
  species on archaeological sites with any degree of accuracy. The regional 
...
# TODO: drop LGM and HS1 since there are no sites 
# (can then maybe increase prediction resolution?)
...
# TODO: better pallette
ggplot() + 
  annotation_spatial(ne_land, fill = "#ffffff") +
  annotation_spatial(ne_rivers, aes(linewidth = strokeweig), colour = "blue") +
  annotation_spatial(ne_lakes, colour = "blue", fill = "lightblue") +
...
flora |>
  ...
  mutate(label = if_else(.estimate < 0.80, taxon, NA_character_)) |>
...
Model accuracy (i.e. proportion of correctly classified test samples) ranged between `r percent(filter(niche_test_metrics, .metric == "accuracy")$min)` and `r percent(filter(niche_test_metrics, .metric == "accuracy")$max)`, with an average of `r percent(filter(niche_test_metrics, .metric == "accuracy")$mean)`.
...
Model accuracy is loosely correlated with the number of occurrences available for training (@fig-model-accuracy-vs-n_occ), though the worst-performing models also included some species with relatively large sample sizes: <!-- TODO e.g. -->.

...
abstract: |
  ...
  of the palaeodistributions of 81<!-- TODO update --> plant species found regularly in association 
  ...
  species at archaeological sites,
  however [...]
...
n_archaeo_assemb <- nrow(distinct(archaeo_flora, site_name, phase_code))
n_archaeo_site <- nrow(distinct(archaeo_flora, site_name))
```
...
```{r data-flora}
# TODO: nest archaeo flora in flora to make it easier to keep track of taxon transformations
if (!file_exists(derived_data("flora.tsv"))) {
  ...
  archaeo_flora |>
    ...
    summarise(
      ...
      p_present = n_present / n_archaeo_assemb,
      .groups = "drop"
    ) |>
    filter(n_present > min_n_archaeo) |>
...
Our starting point was a list of `r nrow(flora)` taxa (@tbl-taxa) comprising the identifiable species observed at at least two Late Epipalaeolithic/Pre-Pottery Neolithic sites, according to our previous study of the regional archaeobotanical data [@ArranzOtaeguiRoe2023].
This was based on dataset of `r n_archaeo_assemb` assemblages collated from three previously published regional archaeobotanical databases: ADEMNES [@ADEMNES], ORIGINS [@ORIGINS], and COMPAG [@COMPAG].
We did not attempt to distinguish between the source of the remains;
...
So for example occurrences for *Bolboschoenus maritimus* also include those recorded under the older nomenclature *Scirpus maritimus* (see @tbl-taxa).
Domestic species meeting our inclusion criteria were substituted with their wild progenitor(s), where different.

## Occurrence data

```{r data-n-occ} 
if (!file_exists(derived_data("flora_n_occ.tsv"))) {
  ...
  write_tsv(flora, derived_data("flora_n_occ.tsv"))
}
 
flora <- read_tsv(
  derived_data("flora_n_occ.tsv"),
  col_types = cols(
    taxon = col_character(),
    taxon_detail = col_character(),
    n_present = col_double(),
...
@ValaviEtAl2022 also recommend using a large background sample for random forest models.
...
The area under the models' receiver operating characteristic curves (ROC-AUC) was in the range of `r round(filter(niche_test_summary, .metric == "roc_auc")$mean, 3)`±`r round(filter(niche_test_summary, .metric == "accuracy")$sd, 3)`
...
```{r niche-archaeo}
# TODO: need to deal with taxon variants / GBIF normalisation
# TODO: factor out
# TODO: what about negatives?
archaeo_flora_predictions <-  archaeo_flora |>
...
write_tsv(archaeo_flora_predictions, derived_data("archaeo_flora_predictions.tsv"))
...
# TODO missing excluded species
# TODO: redo with:
# - alt. taxons as footnotes
# - check boxes of which (climate?) periods observed in ?
flora |>
  # Summarise archaeo occurrences by period
  
  # Extract model metrics
  unnest(c(niche_test_metrics)) |>
  select(-.estimator, -.config) |>
  pivot_wider(names_from = .metric, values_from = .estimate) |>
  # Excluded samples
  mutate(exclude = (n_occ < min_n_occ) | (taxon == "Avena sterilis")) |>
  # Prepare table
  select(taxon, n_present, n_occ, exclude, accuracy, roc_auc, sens) |>
  arrange(taxon) |>
  # Table
  gt() |>
  cols_label_with(fn = str_to_sentence) |>
  cols_label(
    n_present = "Archaeological",
    n_occ = "Present (GBIF)",
    roc_auc = "ROC-AUC",
    sens = "Sensitivity"
  ) |>
  tab_spanner("Occurrences", columns = c(n_present, n_occ)) |>
  tab_spanner("Model", columns = accuracy:sens) |>
  cols_hide(exclude) |>
  tab_footnote(
...
Though the magnitude of the reduction from prehistory to the present likely reflects a degree of overfitting in the model (discussed further below), fluctuations in modelled range size between the Bølling-Allerød (14.7–12.9 ka), Younger Dryas (12.9–11.7 ka), and Early Holocene (11.7–8.3 ka) are more directly comparable [@fig-range-density].
...
ggplot() +
  ...
  geom_stars(data = select(palaeodist_sum, prediction.TRUE.), 
             na.action = na.omit) +
...
Many taxa that occur (or are predicted to occur) across the 'hilly flanks' today—including most crop progenitors—are reconstructed to have had a significantly more restricted distribution in the terminal Pleistocene/Early Holocene (@fig-sum-palaeodist).
...
```{r fig.height=10, fig-palaeodist-barely-pistachio-levant, eval=FALSE, cache=TRUE}
#| fig-cap: Contraction of reconstructed range of of wild barleys and pistachio in the Levant
palaeodist_hordeum <- do.call(c, c(
  filter(flora, str_starts(taxon, "Hordeum") | taxon == "Pistacia atlantica")$palaeodist,
  list(along = list(taxon = filter(flora, str_starts(taxon, "Hordeum") | taxon == "Pistacia atlantica")$taxon))
))
...
Wild barley (*Hordeum spontaneum*), its relatives, and pistachio (*Pistacia atlantica*) show a contraction of their predicted ranges from the Pleistocene to the Holocene,
concurrent with them being brought into cultivation [@fig-palaeodist-barely-pistachio-levant].
These two species also see marked declines in the archaeobotanical record from the Early PPNA/Early PPNB (where they were amongst the most common taxa) to the Late PPNB and Late Neolithic [@ArranzOtaeguiRoe2023].
...
Is consistent with more "macro" trends such as the reduced range of Hordeum and Pistacia correlating with its reduced abundance in the archaeobotanical record.

# Conclusion

* We present the first continuous, spatially explicit models of the palaeodistributions of N plant species found regularly in association with early agricultural archaeological sites in West Asia
